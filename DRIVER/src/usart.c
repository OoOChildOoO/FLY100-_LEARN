...
void Usart1_Init(uint32_t baudrate)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE); //USART1 时钟初始化  //复用串口时钟
  ...
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //send  
  GPIO_Init(GPIOA, &GPIO_InitStructure);
  //PA9(Rx) 配置成上拉输入 
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //receive
