...
{
	SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8);//选择系统时钟（HCLK/8）作为SysTick定时器的时钟  (SystemCoreClock/8)
	SysTick->CTRL = 0x00;
	fac_us = (SystemCoreClock/8)/1000000; //获取微秒系数  X * fac_us = SysTick->LOAD计数装载值
...
void delay_ms(uint32_t nTime)
{
	...
	while((temp&0x01)&&!(temp&(1<<16))); //循环等待延时结束  //SysTick处于使能状态 and SysTick计数标志位不为1
	SysTick->CTRL = 0x00; //失能SysTick定时器
	SysTick->VAL  = 0x00;//清空计数器
}
...
#ifdef INT_DELAY //开中断延时  //当系统中断较多的时候，中断计时法不准，程序容易跑乱
...
uint32_t micros(void)
{
  ...
  do 
	...
	return (ms * 1000) + (usTicks * 1000 - cycle_cnt) / usTicks;//微秒粗时间 +[微秒计数精确](毫秒计数范围 - SysTick的剩余毫秒计数)/微秒计数范围
}
...
void delay_us(uint32_t nus)
{
	uint32_t Tu = 0;
	Tu = micros();//当前值
	while(micros()-Tu <= nus)//实时值 - 当前值 <= 设定值
	{}
}
