...
typedef struct 
{
	...
	uint8_t fill_flag;
	uint8_t cnt;
	int16_t bufferX[buffer_length];
	int16_t bufferY[buffer_length];
	int16_t bufferZ[buffer_length];
}WindowFilter_XYZ;

//姿态解算
extern FLOAT_XYZ 	 Gyr_degree,Gyr_rad;	                //把陀螺仪的各通道读出的数据，转换成弧度制 和 度
extern INT16_XYZ   MPU9250_ACC_RAW,MPU9250_GYRO_RAW,MPU9250_MAG_RAW;	  //MPU最新一次原始数据
extern INT16_XYZ	 GYRO_OFFSET_RAW,ACC_OFFSET_RAW;			//MPU零漂值
extern FLOAT_XYZ 	 Acc_filt,Gry_filt,Mag_filt;	                  //滤波后的各通道数据
