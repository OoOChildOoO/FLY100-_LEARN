...
*功  能 ：将a[low]与a[high]进行排序，并确定一个元素在排序后的位序
*参  数 ：a  数组首地址
*         low数组最小下标
*         high数组最大下标
*返回值 ：返回元素的位序low
...
#define LIMIT( x,min,max ) ( (x) < (min)  ? (min) : ( (x) > (max) ? (max) : (x) ) )//确定x是否在范围内，不在范围内则设置为最值
...
*备  注 : 无
*******************************************************************************/
void Window_Filter(float data,float *filt_data,WindowFilter_Single *Filter_Set)
{
	float temp=0;
	uint8_t i;
	Filter_Set->buffer[Filter_Set->cnt] = data;
	Filter_Set->cnt++;
	if(Filter_Set->cnt<Filter_Set->window_length && Filter_Set->fill_flag) 
		return;   //数组填不满不计算
	else
		Filter_Set->fill_flag = 0;
	
	for(i=0;i<Filter_Set->window_length;i++)
	{
		temp += Filter_Set->buffer[i];
	}
	if(Filter_Set->cnt>=Filter_Set->window_length) Filter_Set->cnt = 0;
	 *filt_data = temp/Filter_Set->window_length;
}
...
*备  注 : 无
*******************************************************************************/
void Window_FilterXYZ(INT16_XYZ *acc,FLOAT_XYZ *Acc_filt,WindowFilter_XYZ *Filter_Set)
{
	int32_t temp1=0,temp2=0,temp3=0,i;
	Filter_Set->bufferX[Filter_Set->cnt] = acc->X;
	Filter_Set->bufferY[Filter_Set->cnt] = acc->Y;
	Filter_Set->bufferZ[Filter_Set->cnt] = acc->Z;
	Filter_Set->cnt++;      //这个的位置必须在赋值语句后，否则bufax[0]不会被赋值
	if(Filter_Set->cnt<Filter_Set->window_length && Filter_Set->fill_flag) 
		return;   //数组填不满不计算
	else
		Filter_Set->fill_flag = 0;
	for(i=0;i<Filter_Set->window_length;i++)
	{
		temp1 += Filter_Set->bufferX[i];
		temp2 += Filter_Set->bufferY[i];
		temp3 += Filter_Set->bufferZ[i];
	}
	 if(Filter_Set->cnt>=Filter_Set->window_length)  Filter_Set->cnt = 0;
	 Acc_filt->X  = temp1/Filter_Set->window_length;
	 Acc_filt->Y  = temp2/Filter_Set->window_length;
	 Acc_filt->Z  = temp3/Filter_Set->window_length;
}
 ...
 *备  注 : 无
*******************************************************************************/
void  SortWindow_Filter(float data,float *filt_data,WindowFilter_Single *Filter_Set)
{
	float temp=0;
	uint8_t i;
	Filter_Set->buffer[Filter_Set->cnt] = data;
	...
	for(i=1;i<Filter_Set->window_length - 1;i++)
	{
		temp += Filter_Set->buffer[i];
	}
	if(Filter_Set->cnt>=Filter_Set->window_length) Filter_Set->cnt = 0;
	 *filt_data = temp/(Filter_Set->window_length - 2);
}
...
*备  注 : 无
*******************************************************************************/
void  SortWindow_FilterXYZ(INT16_XYZ *acc,FLOAT_XYZ *Acc_filt,WindowFilter_XYZ *Filter_Set)
{
	int32_t temp1=0,temp2=0,temp3=0,i;
	Filter_Set->bufferX[Filter_Set->cnt] = acc->X;
	Filter_Set->bufferY[Filter_Set->cnt] = acc->Y;
	...
	if(Filter_Set->cnt<Filter_Set->window_length && Filter_Set->fill_flag) 
		...
		Filter_Set->fill_flag = 0;
	//printf("MPU9250_MAG_RAW Value :fill_flag=%d\n",Filter_Set->fill_flag);
	//printf("MPU9250_MAG_RAW Value :X=%d  Y=%d  Z=%d\n",Filter_Set->bufferX[Filter_Set->cnt],Filter_Set->bufferY[Filter_Set->cnt],Filter_Set->bufferZ[Filter_Set->cnt]);
	for(i=1;i<Filter_Set->window_length - 1;i++)
	{
		temp1 += Filter_Set->bufferX[i];
		temp2 += Filter_Set->bufferY[i];
	 ...
	 if(Filter_Set->cnt>=Filter_Set->window_length)  Filter_Set->cnt = 0;
	 Acc_filt->X  = (float)(temp1/(Filter_Set->window_length - 2));
	 Acc_filt->Y  = (float)(temp2/(Filter_Set->window_length - 2));
	 Acc_filt->Z  = (float)(temp3/(Filter_Set->window_length - 2));
}
